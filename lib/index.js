...
MapboxGeocoder.prototype = {
   ...
   * @param {Function} [fn] function that's called when the event is emitted. If no function is provided, a Promise is returned that resolves when the event is emitted.
   * @returns {MapboxGeocoder} this;
   */
  on: function(type, fn) {
  ...
  /**
   * Subscribe to an event that happens within the plugin, but only once.
   * @param {String} type name of event. Available events and the data passed into their respective event objects are:
   * - __clear__ `Emitted when the input is cleared`
   * - __loading__ `{ query } Emitted when the geocoder is looking up a query`
   * - __results__ `{ results } Fired when the geocoder returns a response`
   ...
   * - __error__ `{ error } Error as string`
   * @param {Function} [fn] function that's called when the event is emitted. If no function is provided, a Promise is returned that resolves when the event is emitted.
   * @returns {MapboxGeocoder|Promise} this if a function is provided, or a Promise that resolves when the event is emitted if no function is provided.
   */
  once: function(type, fn) {
    ...
    this._eventEmitter.once(type, fn);
    return this;
  },
