...
var spatialFormats = require('./spatial-formats');
...
// Options whose value is an object of sub-options. A caller passing a partial
// object keeps the defaults for the keys they left out, instead of wiping them.
const NESTED_OPTION_KEYS = ['inputTransforms', 'parseExtendedSpatialFormats'];
 ...
 * @param {Boolean} [options.parseExtendedSpatialFormats.commaSeparatedLngLatZoom=false] If `true`, recognize input of the form `lng,lat,zoom` with no spaces, e.g. `6.925882,51.110352,11.31`.
 * @param {Boolean} [options.parseExtendedSpatialFormats.slashSeparatedZoomLatLng=false] If `true`, recognize input of the form `zoom/lat/lng` with no spaces, e.g. `11.31/51.110352/6.925882`.
 * @param {Boolean} [options.parseExtendedSpatialFormats.tile=false] If `true`, recognize XYZ tile coordinates of the form `z/x/y`, e.g. `14/8507/5477`, and resolve them to the center of the tile. Note that an all-integer `z/a/b` input within latitude/longitude range, e.g. `12/45/30`, is valid under both this format and `slashSeparatedZoomLatLng`; when both are enabled such input produces two suggestions, the tile one first.
 * @param {Boolean} [options.parseExtendedSpatialFormats.quadkey=false] If `true`, recognize a quadkey, e.g. `12020332200123`, and resolve it to the center of the quadkey. Be aware that a quadkey is any string of the digits `0`-`3`, so enabling this makes purely numeric searches ambiguous — searching for the postal code `20331`, for example, also produces a quadkey suggestion. Only enable it where numeric-only queries are not expected.
 * @example
...
function MapboxGeocoder(options) {
  const defaultOptions = {
    ...
    getItemValue: function(item) {
      if (item._source === spatialFormats.SOURCE) {
  ...
  NESTED_OPTION_KEYS.forEach(function(key) {
    if (options && options[key]) {
      this.options[key] = extend({}, defaultOptions[key], options[key]);
    }
  }, this);
...
MapboxGeocoder.prototype = {
  ...
  _geocode: function(searchInput) {
    ...
    const spatialFormatRes = spatialFormats.parse(searchInput, this.options.parseExtendedSpatialFormats);
    ...
    request.catch(function(error) {
      ...
      .then(
        ...
        function(res) {
          ...
          // put features found by spatialFormat parsing at the start of the list
          res.features = spatialFormatRes.concat(res.features);
        ...
        function(err) {
          ...
          // results from the localGeocoder and from the extended spatial formats -
          // the latter are parsed locally, so they stay valid when the request fails
          var fallbackFeatures = spatialFormatRes.concat(localGeocoderRes);
