...
MapboxGeocoder.prototype = {
  ...
  _geocode: function(searchInput) {
    ...
    if (
      ...
      mbGeocoderReq = geocoderService.reverseGeocode(config).send();
    } else {
      config = extend(config, { query: searchInput });
      mbGeocoderReq = geocoderService.forwardGeocode(config).send();
    }
    ...
    if (this.options.localGeocoder) {
      ...
      localGeocoderReq = new Promise(function(resolve, reject){
        that.options.localGeocoder(searchInput).then(function(result){
          resolve(result);
        }).catch(function(err){
          reject(err);
        });
  ...
  _geocodeComplete: function(localStatus, mbStatus, config) {
    console.log('geocode complete', localStatus, mbStatus)
    ...
    if (localStatus instanceof Error && mbStatus instanceof Error){
      ...
      console.log('local error only');
      return
    } else if (mbStatus instanceof Error){
      console.log('mb error only');
      ...
      console.log('all clear', localStatus, mbStatus);
      ...
      //   res.features = res.features
      //     ? localResponse.concat(res.features)
      //     : localResponse;
      ...
      //   console.log(err, localGeocoderReq, mbGeocoderReq);
      //   this._loadingEl.style.display = 'none';

      //   // in the event of an error in the Mapbox Geocoding API still display results from the localGeocoder
      //   if (localGeocoderReq.length) {
      //     this._clearEl.style.display = 'block';
      //   } else {
      //     this._clearEl.style.display = 'none';
      //     this._typeahead.selected = null;
      ...
      //   this._eventEmitter.emit('results', { features: localGeocoderReq });
      //   this._typeahead.update(localGeocoderReq);
