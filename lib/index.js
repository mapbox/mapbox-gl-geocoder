...
/**
 * @typedef {object} MapboxGeocoderFeature
 * @property {string} id
 ...
 * @property {Array<{id: string, text: string}>} [context]
 * @property {object} [properties]
 * @property {string} [matching_text]
 * @property {string} [matching_place_name]
 * @property {string} [_source]
 * @property {[number, number]} [user_coordinates]
 */

/**
 * @typedef {{ longitude: number, latitude: number }} MapboxGeocoderProximity
 */

/**
 * @callback MapboxGeocoderFilterFunction
 * @param {MapboxGeocoderFeature} feature
 * @returns {boolean}
 */

/**
 * @callback MapboxGeocoderLocalGeocoderFunction
 * @param {string} query
 * @returns {MapboxGeocoderFeature[]}
 */

/**
 * @callback MapboxGeocoderExternalGeocoderFunction
 * @param {string} query
 * @param {MapboxGeocoderFeature[]} features
 * @returns {Promise<MapboxGeocoderFeature[]>}
 */

/**
 * @callback MapboxGeocoderRenderFunction
 * @param {MapboxGeocoderFeature} feature
 * @returns {string}
 */

/**
 * @callback MapboxGeocoderGetItemValueFunction
 * @param {MapboxGeocoderFeature} feature
 * @returns {string}
 */

/**
 * @typedef {object} MapboxGeocoderOptions
 * @property {string} accessToken Required. A Mapbox access token.
 * @property {string} [origin='https://api.mapbox.com'] Use to set a custom API origin.
 * @property {import('mapbox-gl') | null} [mapboxgl] A mapbox-gl instance to use when creating Markers.
 * @property {number} [zoom=16] On geocoded result what zoom level should the map animate to.
 * @property {boolean | object} [flyTo=true] If false, animating the map to a selected result is disabled.
 * @property {string} [placeholder='Search'] Override the default placeholder attribute value.
 * @property {MapboxGeocoderProximity | 'ip'} [proximity] A geographical point for biasing results.
 * @property {boolean} [trackProximity=true] If true, proximity dynamically updates based on the map view.
 * @property {boolean} [collapsed=false] If true, the geocoder control will collapse until hovered or in focus.
 * @property {boolean} [clearAndBlurOnEsc=false] If true, the geocoder clears and blurs on Escape.
 * @property {boolean} [clearOnBlur=false] If true, the geocoder clears its value on blur.
 * @property {[number, number, number, number]} [bbox] A bounding box as [minX, minY, maxX, maxY].
 * @property {string} [countries] A comma separated list of country codes.
 * @property {string} [types] A comma separated list of types to filter results.
 * @property {number} [minLength=2] Minimum number of characters to trigger results.
 * @property {number} [limit=5] Maximum number of results to show.
 * @property {string} [language] IETF language tag(s) for response text and weighting.
 * @property {MapboxGeocoderFilterFunction} [filter] A function to filter results before display.
 * @property {MapboxGeocoderLocalGeocoderFunction} [localGeocoder] A function for local geocoding.
 * @property {MapboxGeocoderExternalGeocoderFunction} [externalGeocoder] A function for external geocoding.
 * @property {'distance' | 'score'} [reverseMode='distance'] Factors for sorting nearby results.
 * @property {boolean} [reverseGeocode=false] If true, enable reverse geocoding mode.
 * @property {boolean} [flipCoordinates=false] If true, reverse geocoding uses lon,lat instead of lat,lon.
 * @property {boolean} [enableEventLogging=true] Allow Mapbox to collect anonymous usage statistics.
 * @property {boolean | import('mapbox-gl').MarkerOptions} [marker=true] If true, a Marker is added at the result location.
 * @property {MapboxGeocoderRenderFunction} [render] A function to render results in the dropdown.
 * @property {MapboxGeocoderGetItemValueFunction} [getItemValue] A function to render the selected result in the search bar.
 * @property {string} [mode='mapbox.places'] The geocoding endpoint to query.
 * @property {boolean} [localGeocoderOnly=false] If true, only localGeocoder results are returned.
 * @property {boolean} [autocomplete=true] Whether to return autocomplete results.
 * @property {boolean} [fuzzyMatch=true] Whether to attempt approximate matching.
 * @property {boolean} [routing=false] Whether to request navigation metadata.
 * @property {string} [worldview='us'] Filter results by worldview.
 * @property {boolean} [enableGeolocation=false] If true, enable user geolocation feature.
 * @property {boolean} [useBrowserFocus=false] If true, use browser focus for suggestions.
 * @property {'address' | 'street' | 'place' | 'country'} [addressAccuracy='street'] Accuracy for geolocation address.
 */

/**
 * @typedef {object} MapboxGeocoderLoadingEvent
 * @property {string} query
 */

/**
 * @typedef {object} MapboxGeocoderResultsEvent
 * @property {MapboxGeocoderFeature[]} features
...
/**
 * @typedef {object} MapboxGeocoderResultEvent
 * @property {MapboxGeocoderFeature} result
 */

/**
 * @typedef {object} MapboxGeocoderErrorEvent
...
class MapboxGeocoder {
  ...
  /** @private */
  _setupConfig(requestType, search) {
   ...
   * @returns {MapboxGeocoder} this
   */
  setInput(searchInput, showSuggestions) {
   ...
   * @returns {MapboxGeocoder} this
   */
  setProximity(proximity, disableTrackProximity = true) {
