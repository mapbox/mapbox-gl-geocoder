...
var accessToken = localStorage.MapboxAccessToken || process.env.MapboxAccessToken;
var baseClient = MapboxClient({ accessToken: accessToken });
var geocoderService = mbxGeocoder(baseClient);
 ...
 * @param {Array} [config.proximity] a proximity argument: this is
...
function MapboxGeocoder(options, config) {
...
MapboxGeocoder.prototype = {
  ...
  onAdd: function(map) {
    this._map = map;
    this.mapboxClient = geocoderService;
  ...
  _geocode: function(searchInput) {
    ...
    this.config.bbox = this.options.bbox;
    ...
    if (/^(\-?\d+(\.\d+)?),\s*(\-?\d+(\.\d+)?)$/.test(searchInput)) {
      // parse coordinates
      var coords = searchInput.split(',').map(function(c){ return parseInt(c, 10) });
      this.config.query = coords;
      // we get an error if result limit isn't 1 for reverse geocodes
      this.config.limit = 1;
      request = geocoderService.reverseGeocode(this.config).send();
    } else {
      this.config.query = searchInput;
      request = geocoderService.forwardGeocode(this.config).send();
    }
  ...
  setProximity: function(proximity) {
    this.config.proximity = proximity;
    return this;
  },
