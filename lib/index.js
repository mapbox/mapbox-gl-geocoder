 ...
 * @param {Object} geocoderApi Any geocoder api that supports the functions reverseGeocode and forwardGeocode and returns a FeatureCollection of results
 ...
 * @param {Boolean|Object} [options.showResultMarkers=true]  If `true`, [Markers](https://docs.mapbox.com/mapbox-gl-js/api/#marker) will be added to the map at the location the top results for the query.   If the value is an object, the marker will be constructed using these options. If `false`, no marker will be added to the map. Requires that `options.mapboxgl` also be set.
...
MaplibreGeocoder.prototype = {
  options: {
    ...
    render: function (item) {
      ...
      return (
        '<div class="maplibregl-ctrl-geocoder--suggestion"><div class="maplibregl-ctrl-geocoder--suggestion-title">' +
        placeName[0] +
        '</div><div class="maplibregl-ctrl-geocoder--suggestion-address">' +
  ...
  onAdd: function(map) {
    ...
    el.className = 'maplibregl-ctrl-geocoder maplibregl-ctrl';
  ...
  _onKeyDown: function(e) {
    ...
    if (e.keyCode === 13) {
      if (this.options.showResultMarkers) {
        ...
        if (results.length > 0 && this._mapboxgl){
      ...
      .then(
        function(response) {
          ...
          if (this.options.externalGeocoder) {
            externalGeocoderRes = this.options.externalGeocoder(searchInput, res.features, config) || [];
  ...
  _collapse: function() {
    ...
    if (!this._inputEl.value && this._inputEl !== document.activeElement) this.container.classList.add('maplibregl-ctrl-geocoder--collapsed');
  },
  ...
  _renderError: function(){
    var errorMessage = "<div class='maplibre-gl-geocoder--error'>There was an error reaching the server</div>"
    this._renderMessage(errorMessage);
  },

  _renderNoResults: function(){
    var errorMessage = "<div class='maplibre-gl-geocoder--error maplibre-gl-geocoder--no-results'>No results found</div>";
    this._renderMessage(errorMessage);
  },
   ...
   * @param {object} geocoderApi An API which contains reverseGeocoding and forwardGeocoding functions to be used by this plugin
   * @returns {MaplibreGeocoder} this
   */
  ...
  _handleResultMarkers: function(results){
    ...
    var markerOptions = extend({}, defaultMarkerOptions, this.options.showResultMarkers);

    results.forEach(function(result) {
      var marker = new this._mapboxgl.Marker(markerOptions);
      if (result.center) {
        marker
