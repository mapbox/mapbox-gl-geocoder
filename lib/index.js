 ...
 * @param {Object} [options.autoFill] An object mapping query selectors of form inputs to parts of selected result. When a result is selected, the form elements will be automatically populated with the result data. 
...
MapboxGeocoder.prototype = {
  ...
  _onChange: function() {
    ...
    if (selected  && JSON.stringify(selected) !== this.lastSelected) {
      ...
      if(typeof this.options.autoFill === "object") {
        this._autoFill(selected);
      }
  ...
  _autoFill: function (selectedResult) {
    for (let qSelector in this.options.autoFill) {
      const resultPart = this.options.autoFill[qSelector];
      const resultText = selectedResult.autoFill[resultPart];
      ...
      if (typeof element === 'object') {
        ...
        nativeInputValueSetter.call(element, resultText);
      ...
      .then(
        ...
        function(res) {
          ...
          res.features.map((feature) => {
            ...
            const address = place_name.split(",")[0];
            ...
            feature.context.forEach((ctx)=>{
              const layer = ctx.id.split('.')[0];
              addrInfo[layer] = ctx.text;
            });
            feature.autoFill = addrInfo;
            return feature;
          });
   ...
   * @returns {Object} The object mapping query selectors (keys) to parts of the selected search result (values)
   */
  getAutoFill: function(){
    return this.options.autoFill;
  },

  /**
   * Set the autoFill object mapping parts of the selected search result to query selectors
   * @param {Object} An object mapping query selectors (keys) to parts of the selected search result (values)
   * @returns {MapboxGeocoder}
   */
  setAutoFill: function(obj){
    this.options.autoFill = obj;
    return this;
  },
