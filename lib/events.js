...
MapboxEventManager.prototype = {
   ...
   * @param {Function} callback 
   */
  keyevent: function(keyEvent, geocoder, callback){
    //pass invalid event
    if (!keyEvent.key) return callback(null); 
    // don't send events for keys that don't change the input
    if (keyEvent.metaKey || [9, 27, 37, 39, 13, 38, 40].indexOf(keyEvent.keyCode) !== -1) return callback(null);
