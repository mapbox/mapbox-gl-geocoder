...
test("Geocoder#inputControl", function (tt) {
  ...
  tt.test("input", function (t) {
    ...
    geocoder.on(
      ...
      once(function (e) {
        ...
        t.equals(
          e.query,
          "-122.47846999999996, 37.81914000000006",
  ...
  tt.test(
    "get language when a language is provided in the options",
    function (t) {
      t.plan(1);
  ...
  tt.test(
    "get language when a language obtained from the browser",
    function (t) {
      t.plan(3);
      setup({});
      t.ok(geocoder.options.language, "language is defined");
      t.ok(
        typeof geocoder.options.language,
        "string",
        "language is defined  as a string"
      );
      t.ok(
        geocoder.options.language.split("-").length,
        2,
        "language is defined as an iso tag with a subtag"
      );
    }
  );
  ...
  tt.test(
    "placeholder language localization with more than one language specified",
    function (t) {
      t.plan(1);
  ...
  tt.test(
    "options.clearAndBlurOnEsc=true clears and blurs on escape",
    function (t) {
      t.plan(4);
      setup({
  ...
  tt.test(
    "options.clearAndBlurOnEsc=false does not clear and blur on escape",
    function (t) {
      t.plan(2);
      setup({
  ...
  tt.test("options.clearOnBlur=true", function (t) {
    ...
    t.equal(
      focusSpy.calledOnce,
      true,
      "called once, focus should not get re-set on input"
    );
...
test("Geocoder#addTo(String) -- no map", function (tt) {
  var container, geocoder, geocoderApi;
